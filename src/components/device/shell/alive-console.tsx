...
export function AliveConsole({
  ...
  // Composer hidden as of P11 — Path A. State retained as no-ops
  // for now since some downstream code paths still reference them
  // through /app/agents/[id]. v1.2 removes the dead state entirely.
  ...
  return (
    <div className={`flex flex-col gap-3 h-full ${className ?? ""}`}>
      ...
      {/* Composer / graph canvas hidden as of P11 (Path A).
      ...
      {/* Composer / builder modal removed (Path A · P11). Routes
          /app/agents/[id], /app/inbox, /app/settings still resolve
          for any pre-existing graph agents. The canvas is off-stage. */}
    </div>
  );
}
