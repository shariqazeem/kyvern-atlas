export default function CustomersLoading() {
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="h-5 w-28 bg-[var(--surface-3)] rounded animate-pulse" />
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="rounded-lg border border-black/[0.06] bg-white p-4 space-y-2">
            ...
            <div className="h-6 w-16 bg-[var(--surface-3)] rounded animate-pulse" />
          </div>
        ))}
      </div>
      <div className="flex gap-2">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="h-8 w-20 bg-[var(--surface-2)] rounded-full animate-pulse" />
        ))}
      </div>
      <div className="rounded-lg border border-black/[0.06] bg-white">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="flex items-center gap-4 px-5 py-3 border-b border-black/[0.03] last:border-0">
            <div className="w-6 h-6 bg-[var(--surface-2)] rounded-full animate-pulse" />
            <div className="h-3 w-24 bg-[var(--surface-2)] rounded animate-pulse" />
            <div className="h-3 w-16 bg-[var(--surface-2)] rounded animate-pulse ml-auto" />
          </div>
        ))}
      </div>
    </div>
